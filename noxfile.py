...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/helm/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/helm/*")
            ...
            #    "--include=src/saltext/helm/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/helm/config/schemas",
    )
